...
# initial_pressure_air = '${units 0.51 Pa}' # Hydrogen in atmosphere is negligible?
initial_pressure_air = '${units 0 psi -> Pa}'
# initial_pressure_gas = '${units 0.24 psi -> Pa}' # Anywhere from 1-10% of 24 psi
...
# ideal gas law or sievert's law could be used for initial concentration???
...
[InterfaceKernels]
  [Equilibrium_gas_to_steel]
    ...
    temperature = '${fparse temperature*1e-6}' # Correction to give proper units to ideal gas constant R
    Ea = '${solubility_activation_energy_in_steel}'
    neighbor_var = H_mobile_gas
    variable = H_mobile_steel
    boundary = interface_steel_to_gas
    diffusivity = '${diffusivity_H_in_steel}'
    # unit_scale = 1e6 # micro
    # unit_scale_neighbor = 1e6 #micro
    # sorption_penalty = 1
    # flux_penalty = 1
...
[BCs]
  [steel_air_boundary] # Boundary of outside edge of steel and open air
    ...
    variable = H_mobile_steel
    # temperature = '${temperature}'
    temperature = '${fparse temperature*1e-6}' # Correction to give proper units to R
    p = 0.5 #Sievert's Law
  []
[]
